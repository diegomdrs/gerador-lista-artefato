...
const ArtefatoFoo = require('../models/artefato-foo')
...
module.exports = function (params) {
  ...
  gerador.gerarListaArtefato = async function () {
    ...
    if (params.projeto && params.autor && params.task && params.diretorio) {
      ...
      try {
        ...
        const listaTarefaFoo = foo(listaArtefatoSemTarefaMesmoTipo)

        return {
          listaArtefato: listaArtefatoComTarefaMesmoTipo,
          listaTarefaFoo: listaTarefaFoo
        }
      } catch (error) {
  ...
  function foo(listaArtefatoSemTarefaMesmoTipo) {

    let listaTarefaFoo = []

    gerador.listaTarefaComSaida.forEach(tarefaParam => {

      let tarefaFoo = new TarefaFoo(tarefaParam)
      ...
      listaArtefatoSemTarefaMesmoTipo.forEach(artefato => {
        ...
        artefato.listaTarefa.filter(tarefa =>
          tarefa.numTarefa === tarefaParam).forEach(tarefa => {
            ...
            const artefatoFoo = new ArtefatoFoo(
              tarefa.tipoAlteracao,
              tarefa.numeroAlteracao,
              artefato.nomeArtefato
            )

            tarefaFoo.listaArtefatoFoo.push(artefatoFoo)
          })
      })

      listaTarefaFoo.push(tarefaFoo)
    })

    return listaTarefaFoo
  }
  ...
  */
  ...
  M	foo-estatico/src/lista-bar.tpl.html
  A	foo-estatico/src/lista-bar.tpl.html
  ---
  ...
  'M' só aparece o arquivo só tiver sido modificado uma vez
  */
