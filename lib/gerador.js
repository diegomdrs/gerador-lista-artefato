...
module.exports = (params) => {
  ...
  function obterListaSaidaArtefatosUmaTarefa(listaArtefatoSemTarefaMesmoTipo) {
    ...
    return params.task.reduce((accumListaSaida, tarefaParam) => {
      ...
      const listaTipoArtefato = listaArtefatoTarefa.reduce((listaRetorno, artefato) => {
        listaRetorno.add(artefato.tipoArtefato)
        return listaRetorno
      }, new Set())
  ...
  function obterTipoArtefato(nomeArtefato) {
    ...
    const extensao = path.basename(nomeArtefato)

    return {
      extensao: extensao,
      tipo: Object.values(TIPO_FILENAME_ARTEFATO).find((tipo) => tipo.regex.test(extensao))
    }
  }
