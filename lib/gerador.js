    ...
    return listaCommitArquivo.reduce(function (accum, arquivoReduce) {
      ...
      if (accum.length === 0) {
        ...
        let artefatoEncontrado = accum.find(artefato =>
          artefato.nomeArtefato === arquivoReduce.nomeArtefato)

        if (artefatoEncontrado) {

          let tarefaModificacaoEncontrada = artefatoEncontrado.listaTarefa.find(tarefa =>
            tarefa.numTarefa === arquivoReduce.commit.numTarefa && tarefa.isTipoAlteracaoModificacao()
          )

          if (tarefaModificacaoEncontrada && arquivoReduce.commit.isTipoAlteracaoModificacao()) {
            tarefaModificacaoEncontrada.numeroAlteracao += 1
          } else {
