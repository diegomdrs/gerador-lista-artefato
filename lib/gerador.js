...
module.exports = (params) => {
  ...
  function obterListaSaidaArtefatosUmaTarefa(listaArtefatoSemTarefaMesmoTipo) {
    ...
    return params.task.reduce((accumListaSaida, tarefaParam) => {
      ...
      const listaProjeto = listaArtefatoTarefa.reduce((accum, artefato) => {
        accum.add(artefato.nomeProjeto)
        return accum
      }, new Set())

      // Filtra as extensões que estão na lista de artefatos que contem a tarefa do loop
      const listaExtensao = listaArtefatoTarefa.reduce((accum, artefato) => {
        accum.add(artefato.extensao)
        return accum
      }, new Set())

      // Filtra as extensões que estão na lista de artefatos que contem a tarefa do loop
      const listaTipoArtefato = listaArtefatoTarefa.reduce((listaRetorno, artefato) => {
        listaRetorno.add(foo(artefato.nomeArtefato))
        return listaRetorno
      }, new Set())
      ...
      for (const nomeProjeto of listaProjeto) {
        ...
        for (const tipoAlteracao of Object.values(TIPO_MODIFICACAO)) {
          ...
          for (const extensao of listaExtensao) {
            ...
            saida.listaArtefatoSaida = listaArtefatoPorExtensao.reduce((accum, artefato) => {
  ...
  function foo(nomeArtefato) {

    const bar = path.basename(nomeArtefato)

    return Object.values(TIPO_FILENAME_ARTEFATO).find((tipo) => 
      tipo.regex.test(bar)
    )
  }
  ...
  async function obterListaPromiseComandoGit() {
    ...
    return params.projeto.reduce((accum, caminhoProjeto) => {
      ...
      if (fs.existsSync(caminhoProjeto)) {
        ...
        accum.push(exec(Comando(caminhoProjeto, params.autor, params.task,
