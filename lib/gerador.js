...
module.exports = function (params) {
  ...
  function obterListaSaidaArtefatosUmaTarefa(listaArtefatoSemTarefaMesmoTipo) {
    ...
    return params.task.reduce((accumListaTarefaComSaida, tarefaParam) => {
      ...
      for (const artefato of listaArtefatoSemTarefaMesmoTipo) {
        
        const foo = artefato.listaTarefa.reduce((accum, tarefa) => {
        ...
        saida.listaArtefatoSaida.push.apply(saida.listaArtefatoSaida, foo)
      }
  ...
  */
  ...
  function obterListaCommitArquivo(saida, nomeProjeto) {
    ...
    return listaRetorno.reduce((accum, retorno) => {

      const numTarefa = retorno.match(/[^\r\n]+/g)[0].match(/\d+/)[0]

      // Ignora a primeira linha
      const listaArquivo = retorno.match(/[^\r\n]+/g).slice(1)

      for (const arquivo of listaArquivo) {
        accum.push(new Arquivo(nomeProjeto, numTarefa, arquivo))
      }
