...
const { TIPO_ARTEFATO } = require('./constants')
...
module.exports = class GeradorPorTarefa extends Gerador {
  ...
  obterListaSaidaTarefa(listaArtefatoAgrupadoPorTarefa) {
    ...
    return listaArtefatoAgrupadoPorTarefa.reduce((accum, tarefa) => {
      ...
      // Verifica se existe algum artefato na tarefa que seja diferente de OUTROS
      // Ou seja, se existe algum arquivo *.yaml ou karma.js, por exemplo
      // if (tarefa.listaArtefato.some((artefato) => artefato.tipoArtefato.tipo !== TIPO_ARTEFATO.OUTROS)) { }

      const saida = this.obterFoo(tarefa)
      accum.push(saida)
  ...
  obterListaSaidaTarefaOld(listaArtefatoAgrupadoPorTarefa) {
