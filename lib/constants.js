module.exports = Object.freeze({
    ...
    TIPO_ARTEFATO_POR_TAREFA: {
        ...
        // 5.15.8 - Alteração de arquivo de definição Dockerfile
        ALTERACAO_ARQ_DOCKERFILE: [
            { regex: /Dockerfile.json$/gi }
        ],
        // 5.15.10 - Alteração de arquivo de definição "Docker Compose"
        DOCKER_COMPOSE_CONFIG: [
            { regex: /docker-compose.yml$/gi }
        ],
        // 5.15.14 - Alteração de objetos de integração e negócio Node.js/GoLang/Kotlin
        OBJ_INTEGRACAO_CLOUD: [
            { regex: /.*(package|package-lock|npm-shrinkwrap).json$/gi }
        ],
        // Tem que ser sempre o último
        OUTROS: [
            { regex: /.+/g }
