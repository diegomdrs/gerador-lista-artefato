...
module.exports = function (params) {
  ...
  gerador.gerarListaArtefato = async function () {
    ...
    try {
      ...
      let listaTarefaArtefato = await executarListaPromiseComando(listaPromiseComando)

      tratarArtefatoRenomeado(listaTarefaArtefato)

      let listaTarefaAgrupadaPorArtefato = agruparTarefaPorArtefato(listaTarefaArtefato)
  ...
  */
  ...
  function agruparTarefaPorArtefato(lista) {

    return lista.reduce(function (accum, artefatoReduce) {

      const novaTarefa = new Tarefa(
        artefatoReduce.tarefa.numTarefa,
        artefatoReduce.tarefa.tipoAlteracao)

      const novoArtefato = new Artefato(
        artefatoReduce.nomeArtefato,
        artefatoReduce.nomeNovoArtefato,
        artefatoReduce.nomeAntigoArtefato,
        artefatoReduce.nomeProjeto,
        [novaTarefa])
      ...
      if (accum.length === 0) {
        ...
        let artefatoEncontrado = accum.find(artefato =>
          artefato.nomeArtefato === artefatoReduce.nomeArtefato)

        if (artefatoEncontrado) {

          let taskModificacaoEncontrada = artefatoEncontrado.listaTarefa.find(tarefa =>
            tarefa.numTarefa === artefatoReduce.tarefa.numTarefa && tarefa.isTipoAlteracaoModificacao()
          )

          if (taskModificacaoEncontrada && artefatoReduce.tarefa.isTipoAlteracaoModificacao()) {
            taskModificacaoEncontrada.numeroAlteracao += 1
          } else {

            if(artefatoReduce.tarefa.isTipoAlteracaoRenomear()) {
              artefatoEncontrado.nomeNovoArtefato = artefatoReduce.tarefa.nomeNovoArtefato,
              artefatoEncontrado.nomeAntigoArtefato = artefatoReduce.tarefa.nomeAntigoArtefato
            }
  ...
  function obterListaArtefato(retorno, nomeProjeto) {
    ...
    return listaRetorno.map(function (f) {
      ...
      let tarefa = new Tarefa(numTarefa, tipoAlteracao)

      if (tarefa.isTipoAlteracaoRenomear()) {

        tarefa.nomeAntigoArtefato = nomeArtefato
        tarefa.nomeNovoArtefato = arquivoArtefato.match(/[^\s]*.$/g)[0]
          .replace(/^/g, nomeProjeto + '/')
      }
  ...
  function tratarArtefatoRenomeado(listaArtefato) {
    ...
    let listaArtefatoTarefaRenomear = listaArtefato.filter(
      artetafoFilter => artetafoFilter.tarefa.isTipoAlteracaoRenomear())

    for (const artefatoRenomeado of listaArtefatoTarefaRenomear) {
      ...
      foo.forEach(artefato =>
        artefato.nomeArtefato = artefatoRenomeado.tarefa.nomeNovoArtefato)
    }
  }
