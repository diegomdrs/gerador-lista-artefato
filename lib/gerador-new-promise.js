...
module.exports = function (params) {
  ...
  gerador.gerarListaArtefato = async function () {
    ...
    try {
      const listaFoo = await obterListaComandoExecutado()
      const listaComando = await bar(listaFoo)

      let listaArtefato = obterListaTarefaAgrupadaPorArtefato(listaComando)
  ...
  */
  ...
  async function obterListaComandoExecutado() {
    ...
    for (const nomeProjeto of params.projeto) {
      ...
      listaPromise.push(exec(comando))
    }

    return listaPromise
  }

  async function bar(listaPromise) {

    let listaArtefato = []

    await Promise.all(listaPromise).then(function (listaRetorno) {

      listaRetorno.forEach(function (retorno, index) {

        const lista = foo(retorno.stdout, params.projeto[index])
  ...
  function foo(retorno, nomeProjeto) {

    const foo = retorno.split(/\n{2,}/g)

    return foo.map(function (f) {
