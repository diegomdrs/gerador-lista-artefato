...
module.exports = function (params) {
  ...
  gerador.gerarListaArtefato = async function () {
    ...
    try {
      const saidaComandoExecutado = await obterListaComandoExecutado()
      const listaComandoComStout = foo()
  ...
  */
  ...
  function obterListaArtefatoTask({ task, nomeProjeto, stdout }) {
    ...
    const listaArtefatosSaidaComando = stdout.match(/[^\r\n]+/g)
  ...
  async function obterListaComandoExecutado() {
    ...
    for (const nomeProjeto of params.projeto) {
      ...
      saidaComandoExecutado =
        saidaComandoExecutado.concat(retorno)
    }

    return saidaComandoExecutado
  }

  function foo(saidaComandoExecutado) {

    var foo = retorno.stdout.split(/\n{2,}/g)

    const bar = foo.map(function (f) {
    ...
    listaSaidaComandoExecutado.push.apply(listaSaidaComandoExecutado, bar)
  }
}
